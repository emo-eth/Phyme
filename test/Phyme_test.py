...
class PhymeTest(unittest.TestCase):
    ...
    def test_perfect_rhymes(self):
        ...
        self.assertTrue(
            "opal" in flatten(self.rd.get_perfect_rhymes("constantinople").values())
        )
    ...
    def test_subtractive_rhymes(self):
        ...
        self.assertTrue(
            "blue" in flatten(self.rd.get_subtractive_rhymes("bloom").values())
        )
        self.assertTrue(
            "matter" in flatten(self.rd.get_subtractive_rhymes("master").values())
        )
    ...
    def test_multisyllable_rhymes(self):
        self.assertTrue(
            "zoology"
            in flatten(
                self.rd.get_perfect_rhymes("toxicology", num_syllables=3).values()
            )
        )
        self.assertFalse(
            "zoology"
            in flatten(
                self.rd.get_perfect_rhymes("toxicology", num_syllables=4).values()
            )
        )

    def test_substitution_rhymes(self):
        self.assertTrue(
            "faster" in flatten(self.rd.get_substitution_rhymes("factor").values())
        )
