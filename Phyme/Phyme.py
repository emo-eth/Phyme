...
from Phyme.constants import PhymeResult
from Phyme.rhymeUtils import PhoneUtils as pu
from Phyme.util import flatten
from Phyme.rhymeUtils import PermutedPhone, Permutation, Phone, StringPhone
from Phyme.RhymeTrieNode import RhymeTrieNode
from Phyme.songStats import sort_words
...
class Phyme(object):
    ...
    def search_permutations(self, phones) -> PhymeResult:
        ...
        result_set: Set[str] = {word for node in nodes for word in node.get_sub_words()}

        sorted_results = sorted(result_set, key=pu.count_syllables)
        grouped_results = groupby(sorted_results, key=pu.count_syllables)
        return dict((k, list(v)) for k, v in grouped_results)

    def sorted_search(
        self, phones: Iterable[Union[Phone, PermutedPhone]], keyword: str
    ) -> PhymeResult:
    ...
    def get_additive_rhymes(self, word: str, num_syllables: int = -1) -> PhymeResult:
        ...
        phones = pu.get_last_syllables(word, num_syllables)
        flattened_phones = flatten(phones)
        permuted_phones = Permutation.ADDITIVE.map(lambda _: True, flattened_phones)
    ...
    def get_assonance_rhymes(self, word, num_syllables=-1) -> PhymeResult:
        ...
        filtered_phones = filter(lambda phone: phone.is_vowel, flattened_phones)
        permuted_phones = Permutation.ADDITIVE.map(lambda _: True, filtered_phones)
