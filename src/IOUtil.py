...
def load_word_phone_dict():
    ...
    with open(os.path.join(file_path, '../cmudict/cmudict-0.7b.txt'), encoding='latin1') as f:
        for line in f:
            if line.startswith(';;;'):
                continue
            splits = line.split()
...
def load_phone_type_dicts():
    ...
    with open(os.path.join(file_path, '../cmudict/cmudict-0.7b.phones.txt'), encoding='latin1') as f:
        for line in f:
            splits = line.split()
            word = splits[0]
